...
func (s *Service) registerModelsForAuth(a *coreauth.Auth) {
	...
	excludedFromAttr := false
	if a.Attributes != nil {
		if val, ok := a.Attributes["excluded_models"]; ok && strings.TrimSpace(val) != "" {
			excluded = strings.Split(val, ",")
			excludedFromAttr = true
		}
	}
	...
	case "codebuddy":
		...
		models = applyExcludedModels(models, excluded)
	case "opencode":
		if entry := s.resolveConfigOpenCode(a); entry != nil {
			if len(entry.Models) > 0 {
				models = buildOpenCodeConfigModels(entry)
			} else {
				models = executor.FetchOpenCodeModels(context.Background(), a, s.cfg)
			}
			if !excludedFromAttr {
				excluded = entry.ExcludedModels
			}
...
func (s *Service) resolveConfigOpenCode(auth *coreauth.Auth) *config.OpenCodeKey {
	if auth == nil || s.cfg == nil {
		return nil
	}
	var attrBase, attrPassword string
	if auth.Attributes != nil {
		attrBase = strings.TrimSpace(auth.Attributes["base_url"])
		attrPassword = strings.TrimSpace(auth.Attributes["server_password"])
	}
	if attrBase == "" {
		attrBase = config.DefaultOpenCodeBaseURL
	}
	for i := range s.cfg.OpenCode {
		entry := &s.cfg.OpenCode[i]
		cfgBase := strings.TrimSpace(entry.BaseURL)
		if cfgBase == "" {
			cfgBase = config.DefaultOpenCodeBaseURL
		}
		if !strings.EqualFold(cfgBase, attrBase) {
			continue
		}
...
func buildOpenCodeConfigModels(entry *config.OpenCodeKey) []*ModelInfo {
	...
	return buildConfigModels(entry.Models, "opencode", "opencode")
}
